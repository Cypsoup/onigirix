...
function renderRecipeRow($recipe)
{
    ...
    <div class="flex justify-between items-center py-2 border-b border-black/5 text-sm {$opacityClass}">
        ...
        <div class="flex items-center gap-4">
            <button type="button" onclick="document.getElementById('qty-{$id}').stepDown()" class="w-8 h-8 border border-black flex items-center justify-center hover:bg-black hover:text-white transition-colors" {$disabled}>-</button>
...
/**
 * Affiche la barre de navigation du bas pour l'application utilisateur
 * @param string $activePage - L'identifiant de la page actuelle ('dashboardUser', 'orderUser', etc.)
 */
function renderBottomNav($activePage = 'dashboardUser') {
    // Définition de nos 4 boutons
    $navItems = [
        'dashboardUser' => [
            'label' => 'ACCUEIL',
        ...
        'historyUser' => [
            'label' => 'HISTORIQUE',
            'url' => '#', // Mettre le lien de la page historique plus tard
        ...
        'profileUser' => [
            'label' => 'PROFIL',
            'url' => '#', // Mettre le lien de la page profil plus tard
    ...
    // On boucle sur chaque bouton pour le générer
    foreach ($navItems as $key => $item) {
        // Est-ce que le bouton correspond à la page actuelle ?
        $isActive = ($key === $activePage);
        
        // Si oui, on le met en rouge et plus épais. Sinon, en gris clair.
...
function createOrder($pdo, $trigramme, $items, $montant_total)
{
    try {
        ...
        throw $e; // On relance l'exception pour que submit-order.php l'attrape et renvoie une erreur HTTP 400
    }
}
